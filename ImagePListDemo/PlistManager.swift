  ...
  private init() {} //This prevents others from using the default '()' initializer for this class.
    ...
    func startPlistManager() {
        if let _ = Plist(name: plistFileName) {
            print("[PlistManager] PlistManager started for ", plistFileName)
            //print("[PlistManager] PlistManager started for ", plistFileName1)
        }

    }
    
    func getPlist() -> NSDictionary{
        let plist = Plist(name: plistFileName)
    ...
    func printPlist(){
        let plist = Plist(name: plistFileName)
        print(plist?.getValuesInPlistFile() ?? 0)
    }
